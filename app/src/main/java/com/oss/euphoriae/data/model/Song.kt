...
data class Song(
    ...
    /** Stable unique key for Compose lazy lists; [index] disambiguates duplicate entries in queues. */
    fun lazyItemKey(index: Int): String {
        val identity = when {
            data.isNotEmpty() -> data
            id > 0L -> "id:$id"
            else -> "$title|$artist|$album|$duration"
        }
